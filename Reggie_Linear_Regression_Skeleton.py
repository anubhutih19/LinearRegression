...
def calculate_error(m,b,point):
    x_point = point[0]
    y_point = point[1]
    y_at_xpoint = get_y(m,b,x_point)
    val = y_at_xpoint - y_point
    return abs(val)
...
possible_ms = [m * 0.1 for m in range(-100,110)]
...
possible_bs = [m * 0.1 for m in range(-200,210)]
...
best_b = ()
    ...
    for m in possible_ms:
        for b in possible_bs:
            new = calculate_all_error(m,b,datapoints)
            if new < smallest_error:
                smallest_error == new
                best_m == m
                best_b == b
